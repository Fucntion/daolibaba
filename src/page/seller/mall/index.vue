   ...
   <div class="malls">
     <mt-loadmore :bottom-method="getMall" :autoFill="false" @bottom-status-change="handleBottomChange" :bottom-all-loaded="allLoaded" ref="loadmore">
      <div class="mall" v-for="(mall,idx) in malls" :key="idx">
        ...
        <div class="control">
          <mt-button size="small" v-if="mall.status!=3" type="danger">上架</mt-button>
          <mt-button size="small" v-else type="danger">下架</mt-button>
          <router-link :to="'seller/mall/'+mall.id"><mt-button size="small" type="primary">详细信息</mt-button></router-link>
        </div>
      </div>
  ...
  export default {
    ...
    mounted(){
      this.$nextTick(function () {
        document.querySelector('.malls').style.height = window.screen.height;
      })
    },
    methods: {
