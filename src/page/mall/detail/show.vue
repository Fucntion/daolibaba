<template>
  <div>
    ...
    <!--团购信息-->
    <timer-box v-if="mall.group&&mall.group.start_time*1000<now&&mall.group.end_time*1000>now" :mall="mall" :now="now" :start_time="mall.group.start_time*1000" :end_time="mall.group.end_time*1000"></timer-box>
    ...
    <div v-if="mall.id" class="baseinfo">
      ...
      <div class="info-line padding10-c pricebox" v-if="!mall.group">
        ...
        <span class="initial_price">￥<em>{{mall.marketprice|cutPrice(0)}}</em>{{mall.marketprice|cutPrice(1)}}</span>
      </div>
      ...
      <div class="info-line ext">
        ...
        <span @click="toScore()">总评论:{{mall.comments}}</span>
      </div>
    </div>
    <!-- 规格信息 -->
    ...
    <div class="fixBar">
      ...
      <div class="action">
        ...
        <div @click="openSpecBox('cart')" class="btn tocart buyJs">加入购物车({{shopProductCount}})</div>
      </div>
    ...
    <mt-popup v-if="mall.company"
      ...
      position="bottom" class="kefu-box userselect">
      <mt-cell  title="联系电话" >
...
<script>
  ...
  export default {
    ...
    methods: {
      ...mapActions(["getCartProductCount"]),
      toScore(){
        console.log('/mall/detail/'+this.$route.params.id+'/score')
        this.$router.push({
          path:'/mall/detail/'+this.$route.params.id+'/score'
        })
      },
      kefu(){
      ...
      getInfo() {
        ...
        mise.then(res => {
          ...
          if(res.body.code==0&&res.body.data.errorCode==10001){
            //用replace不用path，是因为不需要给history添加历史记录
            this.$router.replace({
              path:'/notfound'
            })
          }
