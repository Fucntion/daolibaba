...
<template>
  <div>
    ...
    <div class="cartbox" v-if="total.totalNumber>1||selectAll">
      <div class="control">
        <mt-button size="small" @click="delMall('all')">全部删除</mt-button>
      </div>
      <div class="section" v-for="(company,index) in companyList" :key="index">
        <div class="head">
          <i class="iconfont icon-select icon-select" :class="{'active':company.select}"
        ...
        <div class="mall selected" v-for="(mall,idx) in company.malls">
          <i v-if="!mall.disabled" class="iconfont icon-select icon-select" :class="{'active':mall.select}"
             @click="selectMall(mall,company)"></i>
          <span class="lefttip" v-else>预热</span>
          ...
          <div class="info">
            ...
            <div class="info_line specinfo" v-if="mall.group">
              <span v-if="mall.group.now>mall.group.start_time&&mall.group.now<mall.group.end_time" class="font12">团购将于{{mall.group.endtime}}截止</span>
              <span v-if="mall.group.now<mall.group.start_time" class="font12">团购将于{{mall.group.start_time}}开始</span>
              <span v-if="mall.group.now>mall.group.end_time" class="font12">团购已经结束</span>
            </div>
            <div class="info_line specinfo" v-if="mall.group">
              <span class="font12">每人限购{{mall.group.limit}}件</span>
            </div>
            <div class="info_line">
              <p class="price"><span class="priceJs">¥ <em class="int">{{mall.price|cutPrice(0)}}</em>{{mall.price|cutPrice(1)}}</span>
              </p>
    ...
    <router-link :to="'/'" class="empty  padding15" v-if="total.totalNumber<1&&!selectAll">
  ...
  export default {
    ...
    methods: {
      ...
      add: function (mall) {
        ...
        if(mall.group&&mall.number == mall.gourp.limit){
          this.$root.mint.alertMsg('该商品每人限购'+mall.group.limit+'件')
          return ;
        }

        if(mall.number+1 ==mall.stock){
          this.$root.mint.alertMsg('已达到最大库存');
          return ;
        }
