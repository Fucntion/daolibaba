...
// export const loginRedirect = name => {
//   if (!name) return;
//   window.localStorage.removeItem(name);
// }
...
export const unifiedPay = function (out_trade_no, channel = 'wx_pub') {
  ...
  let payPromise = new Promise(function (resolve, reject) {
    ...
    pay(post).then(result => {

      let body = result.body;
      if (body.code === 1) {
        //下单成功
        let payInfo = result.body.data;
...
export const getWxConfig = () => {
  ...
  let wxConfigPromise = new Promise(function (resolve, reject) {
    ...
    let mise = getJsSign({
      url: window.__wxjs_is_wkwebview !== true?location.href.split('#')[0]:store.state.wxurl
    });
